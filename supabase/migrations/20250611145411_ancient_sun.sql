/*
  ...
  1. New Tables
    - `sources` - Store uploaded content sources (files, URLs, etc.)
    - `chunks` - Text chunks with embeddings for vector search
    - `threads` - Conversation threads with centroid embeddings
    - `micro_threads` - Individual Q&A interactions within chunks
...
DROP INDEX IF EXISTS chunks_source_id_idx;
DROP INDEX IF EXISTS chunks_thread_id_idx;
...
DROP INDEX IF EXISTS threads_centroid_idx;
...
-- Create index if it doesn't exist
DROP INDEX IF EXISTS micro_threads_parent_chunk_id_idx;
